...
### Cambios de configuración Firebase en PR (dev)

Esta guía aplica cuando un PR en rama `dev` modifica configuración de Firebase que afecta autenticación/frontend (`public/firebase-config.js`) o el deploy automático de `.github/workflows/deploy-by-branch.yml`.

#### 1) Secretos requeridos (exactos)

Para que el job `deploy-dev` funcione sin omitir el despliegue, GitHub Actions necesita todos estos secretos:

- `FIREBASE_SERVICE_ACCOUNT_BINGO_ONLINE_DEV`
- `FIREBASE_DEV_WEB_API_KEY`
- `FIREBASE_DEV_AUTH_DOMAIN`
- `FIREBASE_DEV_DATABASE_URL`
- `FIREBASE_DEV_PROJECT_ID`
- `FIREBASE_DEV_STORAGE_BUCKET`
- `FIREBASE_DEV_MESSAGING_SENDER_ID`
- `FIREBASE_DEV_APP_ID`
- `FIREBASE_DEV_MEASUREMENT_ID` (opcional, pero recomendado)

> El workflow valida además que `FIREBASE_DEV_PROJECT_ID=bingo-online-dev`, `FIREBASE_DEV_AUTH_DOMAIN=bingo-online-dev.firebaseapp.com` y `FIREBASE_DEV_DATABASE_URL=https://bingo-online-dev-default-rtdb.firebaseio.com` antes de desplegar.

#### 2) Generación de `firebase-config.js` en PR

1. Configure/actualice secretos en GitHub (no en el repositorio).
2. En local, cree/actualice `.env.dev` con las variables Firebase de `dev`.
3. Genere el archivo de frontend exactamente así:

```bash
FIREBASE_ENV_FILE=.env.dev npm run generate:firebase-config
```

4. Verifique que `public/firebase-config.js` tenga valores `dev` para `projectId`, `authDomain`, `storageBucket` y `databaseURL`.
5. No commitee secretos, JSON de service account ni archivos `.env*`.

#### 3) Validaciones previas al merge (pre-deploy)

- Confirmar que el PR está dirigido a `dev` y que no cambia secretos de `staging/main` por error.
- Confirmar que el archivo base sigue siendo `public/firebase-config.template.js` y que la generación se hace por script (`npm run generate:firebase-config`).
- Verificar consistencia con el workflow `deploy-by-branch.yml`:
  - rama `dev` → `projectId: bingo-online-dev`
  - rama `dev` → `target: dev`
- Ejecutar pruebas mínimas del repositorio antes de merge:

```bash
npm test
```

#### 4) Validaciones posteriores al deploy (post-deploy)

Tras merge/push en `dev` (que dispara **Deploy by branch to Firebase Hosting**):

1. Revisar logs del job `deploy-dev` en GitHub Actions y confirmar:
   - paso `Validar configuración esperada para dev` en verde;
   - `Deploy to Firebase Hosting` completado para `projectId: bingo-online-dev` y `target: dev`.
2. Abrir `https://bingo-online-dev.web.app/` en incógnito.
3. Validar login Google y navegación a vista por rol.
4. Confirmar en consola del navegador que no aparezcan errores `auth/unauthorized-domain`, `auth/operation-not-allowed` o fallas por proyecto cruzado.

#### 5) Matriz de errores comunes (síntoma → causa probable → corrección)

| Síntoma | Causa probable | Corrección |
|---|---|---|
| Error Google login en `dev` (`auth/unauthorized-domain`) | `FIREBASE_DEV_AUTH_DOMAIN` incorrecto o dominio `bingo-online-dev.web.app` no autorizado en Firebase Auth | Ajustar secreto `FIREBASE_DEV_AUTH_DOMAIN` a `bingo-online-dev.firebaseapp.com` y autorizar `bingo-online-dev.web.app` en Firebase Console > Authentication > Authorized domains. |
| App carga pero usa datos de otro entorno | `FIREBASE_DEV_PROJECT_ID` no corresponde a `bingo-online-dev` | Corregir secreto `FIREBASE_DEV_PROJECT_ID`, regenerar config y redeploy de `dev`. |
| Error al subir archivos o URLs rotas de Storage | `FIREBASE_DEV_STORAGE_BUCKET` apunta a bucket equivocado o formato incorrecto | Corregir `FIREBASE_DEV_STORAGE_BUCKET` con el bucket del proyecto `dev` (respetando formato provisto por Firebase). |
| Error de inicialización Firebase en frontend | `public/firebase-config.js` desactualizado (no regenerado) | Regenerar con `FIREBASE_ENV_FILE=.env.dev npm run generate:firebase-config` y desplegar nuevamente. |
| Workflow omite deploy (`No se encontraron todas las credenciales...`) | Falta uno o más secretos requeridos en repo | Crear/completar secretos faltantes con nombres exactos y relanzar push en `dev`. |
| Falla autenticación admin/scripts backend en `dev` | Credenciales de service account incorrectas o incompletas | Recrear `FIREBASE_SERVICE_ACCOUNT_BINGO_ONLINE_DEV` desde JSON válido del proyecto `bingo-online-dev`. |

#### 6) Criterio de rollback inmediato

Ejecute rollback **inmediato** si se cumple cualquiera de estos criterios tras deploy en `dev`:

- Login Google falla para la mayoría de usuarios de prueba.
- La app en `dev` conecta a proyecto distinto de `bingo-online-dev`.
- Se detecta error de bucket/credenciales que bloquea operación básica (auth, lectura o subida).

Pasos de rollback:

1. Revertir en GitHub Secrets los valores modificados de `dev` a la última versión estable:
   - `FIREBASE_DEV_AUTH_DOMAIN`
   - `FIREBASE_DEV_PROJECT_ID`
   - `FIREBASE_DEV_STORAGE_BUCKET`
   - `FIREBASE_DEV_DATABASE_URL`
   ...
   - `FIREBASE_DEV_APP_ID`
   - `FIREBASE_SERVICE_ACCOUNT_BINGO_ONLINE_DEV`
2. Si el PR también cambió lógica de generación/configuración, revertir commit en rama `dev`.
3. Ejecutar nuevo deploy en `dev` (push vacío o re-run del workflow).
4. Verificar recuperación:
   - workflow `deploy-dev` en verde;
   - login Google exitoso en `https://bingo-online-dev.web.app/`;
   - operaciones básicas sin errores en consola ni Storage/Firestore.
