...
describe('uploadServer variables requeridas', () => {
  ...
  test('validateRequiredEnv falla en producción si UPLOAD_ENDPOINT queda en localhost por defecto', () => {
    const { validateRequiredEnv } = require('../uploadServer.js');
    const exitSpy = jest.spyOn(process, 'exit').mockImplementation(() => {
      throw new Error('process.exit');
    });
  ...
  test('validateRequiredEnv en producción permite endpoint no-local y advierte ALLOWED_ORIGINS localhost', () => {
    const { validateRequiredEnv } = require('../uploadServer.js');
    const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => {});
    ...
    expect(() =>
      validateRequiredEnv({
        ...
        GOOGLE_APPLICATION_CREDENTIALS: '/tmp/serviceAccountKey.json',
        FIREBASE_STORAGE_BUCKET: 'demo.appspot.com',
        UPLOAD_ENDPOINT: 'https://api.bingo.example/upload'
      })
    ).not.toThrow();
